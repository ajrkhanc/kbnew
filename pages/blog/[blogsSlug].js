...
export async function getServerSideProps(context) {
   const post5 = await fetch('https://blanchard-adminsc.herokuapp.com/api/posts')
   const posts5 = await post5.json()

   const cat = await fetch('https://blanchard-adminsc.herokuapp.com/api/categories')
   const cats = await cat.json()


   const posturl = context.params.blogsSlug;
   const res = await fetch(`https://blanchard-adminsc.herokuapp.com/api/post/${posturl}`)
   const posts = await res.json()

   const getfeedback = await fetch(`https://blanchard-adminsc.herokuapp.com/api/posts/feed/${posturl}`)
